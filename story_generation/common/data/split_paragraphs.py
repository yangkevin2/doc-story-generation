...
def split_paragraphs(text, mode='none', max_sentence_length=10000000):
    """
    ...
    if mode == 'none':
        ...
        while '\n\n' in text:
            text = text.replace('\n\n', '\n')
        sentences = sum([[s.strip() for s in tokenize.sent_tokenize(t)] for t in text.split('\n')], [])
        chunks = []
        tokenizer = load_split_paragraph_tokenizer()
        for s in sentences:
            if len(tokenizer.encode(s)) <= max_sentence_length:
                chunks.append(s)
            else:
                chunks += group_chunks(s.split(), max_chunk_length=max_sentence_length)
        return chunks
    else:
